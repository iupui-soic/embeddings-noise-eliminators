#!/usr/bin/env bash
# GPU 1 subset: the two slower, higher-dimensional models.
set -u
...
for nb in "${ORDER[@]}"; do
  ...
  echo "[$(date -Is)] === gpu1 START $name ==="
  if jupyter nbconvert --to notebook --execute --inplace \
        --ExecutePreprocessor.timeout=-1 \
        "$nb" 2>&1 | tee -a "$LOG_ROOT/gpu1_${name}.log"; then
    echo "[$(date -Is)] === gpu1 OK  $name ==="
  else
    echo "[$(date -Is)] === gpu1 FAIL $name ==="
  fi
done
echo "[$(date -Is)] gpu1 ALL EXPERIMENTS FINISHED"

...
for nb in "${ORDER[@]}"; do
  ...
  echo "[$(date -Is)] === mimic-gpu0 START $name ==="
  if jupyter nbconvert --to notebook --execute --inplace \
        --ExecutePreprocessor.timeout=-1 \
        "$nb" 2>&1 | tee -a "$LOG_ROOT/mimic_gpu0_${name}.log"; then
    echo "[$(date -Is)] === mimic-gpu0 OK  $name ==="
  else
    echo "[$(date -Is)] === mimic-gpu0 FAIL $name ==="
  fi
done
echo "[$(date -Is)] mimic-gpu0 ALL EXPERIMENTS FINISHED"

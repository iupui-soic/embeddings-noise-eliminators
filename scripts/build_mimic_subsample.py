#!/usr/bin/env python3
"""
Pre-registered MIMIC-CXR-JPG subsample for v4 manuscript.
...
in the pipeline. Committed to git BEFORE running MIMIC embeddings so the
sampling is pre-registered.

Design (MedIA-defensible):
  ...
  - Probe-training subsampled within the official train split:
        ...
        * seed = 42 (fixed)
  - Output columns kept at image (dicom_id) level so downstream code can
    filter with a single isin() call.
"""
